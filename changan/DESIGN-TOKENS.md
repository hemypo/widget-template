...
Идёт **перед** блоком `:root, :host` в файле, не внутри него (тот же паттерн, что в `haval`):
...
1. При создании/правке любого комплекта в `changan/` — канонический блок `:root, :host {...}` и строки `@font-face`/`.cefalot-form-custom-template-root {...}` перед ним копируются в начало `desktop.css` и `mobile.css` (или единственного `desktop.css`/`mobile.css` многошаговой формы) без изменений.
